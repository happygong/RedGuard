...
if(isset($_GET['delete'])){
    $id = $_POST['id'];
    $row=$DB->query("SELECT * FROM hgfh_blog WHERE id='{$id}' limit 1")->fetch();
    if (!$row) {
        exit('{"code":-1,"msg":"删除失败，该博客不存在！"}');
    }
    $sql=$DB->exec("DELETE FROM hgfh_blog WHERE id='{$id}'");
    if ($sql){
                ...
                <div class="row">
                    <div class="col-12">
                        <div class="page-title-box">
                          ...
                          <a href="./addblog.php" class="btn btn-primary">添加博客</a>
                          </div>
                          <br>
                        </div>
                    </div>
                        ...
                        <div class="card">
                            <div class="card-body">
                                ...
                                <table class="table table-bordered mb-0">
                                    ...
                                    $rs=$DB->query("SELECT * FROM hgfh_blog WHERE 1 order by id asc limit $offset,$pagesize");
                                    while($res = $rs->fetch())
                                    {
                                        $i++;
                                        echo '<tr>
                                            ...
                                            <td><a href="./blogedit.php?edit&id='.$res['id'].'" class="btn btn-xs btn-info">编辑</a>&nbsp;
