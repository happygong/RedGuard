...
if($my=='search') {
    $sql=" `name`='{$_GET['name']}'";
    $numrows=$DB->query("SELECT * from hgfh_sc HERE{$sql}")->rowCount();
}else{
...
if(isset($_GET['delete'])){
    ...
    $sql=$DB->exec("DELETE FROM hgfh_sc WHERE id='{$id}'");
    if ($sql){
...
if(isset($_GET['deleteall'])){
     if($islogin=1){
    $sql=$DB->exec("TRUNCATE TABLE hgfh_sc");
    if ($sql=true){
        exit('{"code":1,"msg":"全部网址删除成功！"}');
    } else {
        exit('{"code":-1,"msg":"删除失败！"}');
    }
     }else{
                ...
                <div class="row">
                    <div class="col-12">
                        <div class="page-title-box">
                            ...
                            <h4 class="page-title">防红网址</h4> 
                           <div style = "text-align:right;"> 
                          <a href="#" class="btn btn-primary deleteall">删除所有网址</a>
                          </div>
                          <br>
                        </div>
                    </div>
                        ...
                        <div class="card">
                            <div class="card-body">
                                ...
                                <table class="table table-bordered mb-0">
                                    ...
                                    $rs=$DB->query("SELECT * FROM hgfh_sc WHERE 1 order by id asc limit $offset,$pagesize");
                                    ...
                                    while($res = $rs->fetch())
                                    {
                                       $url=base64_decode($res['url']);
                                        $i++;
                                      if($res['user']!==NULL){
                                      $user=$DB->query("SELECT * FROM hgfh_user WHERE id='{$res['user']}' limit 1")->fetch();
                                      }
                                        echo '<tr>
                                            <td>'.$i.'</td>
                                            <td>'.$url.'</td>
                                            ...
                                            <td>'.($res['user']==NULL?'<font color=blue>陌生用户</font>':'<font color=black>'.$user['user'].'</font>').'</td>
                                            <td>
