...
require_once("../../include/codepay_config.php"); //导入配置文件
...
$user = $_POST['name'];//提交的用户  也可以去session或者cookie中取登录用户 $_SESSION['uid']
$pay_id = $_POST['name']; //网站唯一标识 需要充值的用户名，用户ID或者自行创建订单 建议传递用户的ID


$price = (float)$_POST["price"]; //提交的价格

$param = $_POST['trade_no']; //自定义参数  可以留空 传递什么返回什么 用于区分游戏分区或用户身份
...
if (!file_exists('/js/codepay_util.js')) { //这个资源文件存在 走本地数据 不存在则走云端数据
...
if($price == ''){
    exit('{"code":-1,"msg":"未输入充值金额！"}');
    
}
if ($price < $codepay_config['min']) exit('{"code":-1,"msg":"最低限制' . $codepay_config['min'] . '元"}'); //检查最低限制
...
function create_link($params, $codepay_key, $host = "")
{
    ...
    $apiHost = ($host ? $host : "http://api4.xiuxiu888.com/creat_order/?"); //网关
...
function getCodepayHtml($parameter,$key,$gateway='http://api4.xiuxiu888.com/creat_order/?',$https_gateway='https://api.xiuxiu888.com/creat_order/?'){
    $codepay_json='';
    if ($parameter['page'] != 3) { //只要不为3 返回JS 就去服务器加载资源
        $parameter['page'] = "4"; //设置返回JSON
        $back = create_link($parameter, $key, $gateway); //生成支付URL
        $timeout = 3; //超时设置 5秒
    ...
    if (empty($codepay_json)) { //如果没有获取到远程HTML 则走JS创建订单
        ...
        $qr='';
        $money=$parameter['price'];
        $back = create_link($parameter, $key, $https_gateway);
...
$qr=$codepay_back_data['qr'];
$codepay_html=$codepay_back_data['codepay_html'];
exit('{"code":1,"code_url":"'.$qr.'","price":"'.$price.'"}');
...
<script src="<?php echo $codepay_path ?>/js/codepay_util.js?v=3.0"></script>
